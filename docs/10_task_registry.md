...
| TS-003A | Integração do Obsidian e metodologia de conhecimento | Organizar contexto, aprendizado e preparação de publicações em um Vault local, sem substituir o repositório oficial. | `VERSIONED` | Amarelo aprovado; vermelho autorizado para versionamento | TS-003 | Aprovada e publicada | `4e2a003` | Preservar como baseline de conhecimento. |
| TS-004 | Asset Registry | Definir e implementar o cadastro canônico de ativos. | `PLANNED` | A definir | TS-003A versionada | Pendente | — | Preparar decisão de arquitetura e contrato de dados. |
